...
describe('Replies API', function () {
  ...
  it('should give an error (400) for an invalid video ID', function (done) {
    this.timeout(20000);
    getReplies('fakeId', 'z13oy5eavyzketqp204cjvjadqu5xttiwhk').then(function (page) {
  ...
  it('should give an error (503) for an invalid comment ID', function (done) {
    this.timeout(20000);
    getReplies('eKEwL-10s7E', 'yadayada').then(function (page) {
      expect(page).not.to.exist;
      done();
    }).catch(function (error) {
      expect(error).to.exist;
  ...
  it('should get replies to a comment', function (done) {
    this.timeout(10000);
    getReplies('eKEwL-10s7E', 'z13oy5eavyzketqp204cjvjadqu5xttiwhk').then(function (page) {
